...
class appointmentController {
  ...
  static async updateAppointment(req, res) {
    try {
      const appointmentId = req.params.id;
      const updateAppointmentBody = req.body;
      const findAppointmentForUpdate = await Appointment.findOne({
        where: { id: appointmentId },
      });
      await findAppointmentForUpdate.update(updateAppointmentBody);
  ...
  static async deleteAppointment(req, res) {
    try {
      const appointmentId = req.params.id;
      const findAppointmentForDelete = await Appointment.findOne({
        where: { id: appointmentId },
      });
      await findAppointmentForDelete.destroy(appointmentId);
      res.status(200).json("Consulta excluída");
    } catch (error) {
